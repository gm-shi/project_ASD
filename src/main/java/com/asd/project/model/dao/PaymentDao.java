...
public class PaymentDao{
    ...
    public List<Payment> findByIDandDATE(Integer id, String date, String email) throws SQLException {
        ...
        if(id == 0){
            sql = "SELECT * FROM PAYMENT WHERE CAST(payment_date) LIKE ? AND email=?";
            pst = this.connection.prepareStatement(sql);
            pst.setString(1, "%" + date + "%");
            pst.setString(2, email);
        } else {
            sql = "SELECT * FROM PAYMENT WHERE payment_id=? AND email=?";
            pst = this.connection.prepareStatement(sql);
            pst.setInt(1, id);
            pst.setString(2, email);
        }
        ...
        if(res.next()) {
            ...
            payment.setEmail(res.getString("email"));
    ...
    public void addPayment(Payment payment) throws SQLException {
        String sql="INSERT INTO PAYMENT (payment_id, email, payment_method, payment_date, card_number, securitycode, expirydate, nameoncard) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public void updatePayment(Payment payment) throws SQLException {
        String sql="UPDATE PAYMENT SET email=?, payment_method=?, payment_date=?, card_number=?, securitycode=?, expirydate=?, nameoncard=? WHERE payment_id=?";
        PreparedStatement pst = this.connection.prepareStatement(sql);
